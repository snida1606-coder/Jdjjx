import asyncio, json, threading, time, ssl, re, os, sys
...
WS_HEADERS = {
    ...
    "Referer": "https://qxbroker.com/pt/trade",
}
...
def login():
    for attempt in range(1, 21):
        ...
        try:
            ...
            r1 = s.get("https://qxbroker.com/pt/trade", timeout=30)
            if r1.status_code != 200: print(f"  Blocked ({r1.status_code})"); time.sleep(3); continue
            m1 = re.search(r'name="_token"\s+value="([^"]+)"', r1.text)
            tok = m1.group(1) if m1 else None
            if not tok:
                s.post("https://qxbroker.com/api/v3/auth/sign-in", json={"email": EMAIL, "password": PASSWORD}, timeout=30)
                r3 = s.get("https://qxbroker.com/pt/trade", timeout=30)
                for sc in re.findall(r'<script[^>]*>(.*?)</script>', r3.text, re.DOTALL):
                    if "window.settings" in sc:
                        ...
                        if d.get("token"):
                            ...
                            print(f"  OK (API)"); return d["token"], cookies
                print(f"  Failed"); time.sleep(3); continue
            time.sleep(2)
            r4 = s.post("https://qxbroker.com/pt/sign-in/", data={"_token": tok, "email": EMAIL, "password": PASSWORD, "remember": 1}, timeout=30)
            for sc in re.findall(r'<script[^>]*>(.*?)</script>', r4.text, re.DOTALL):
                if "window.settings" in sc:
                    ...
                    if d.get("token"):
                        cookies = "; ".join([f"{k}={v}" for k, v in s.cookies.items()])
                        print(f"  OK"); return d["token"], cookies
            print(f"  Failed"); time.sleep(3)
        except Exception as e: print(f"  Error: {e}"); time.sleep(3)
    return None, None
...
async def connect_ws(ssid, cookies=""):
    ...
    ws = await websockets.connect(WS_URL, additional_headers=hdrs, ssl=ssl_ctx, ping_interval=10, ping_timeout=20, max_size=2**23)
    for _ in range(2):
...
async def init():
    ...
    if instruments:
        ...
        print(f"Pairs loaded: {len(_pairs)}")
    else:
...
async def fetch_candles(pair_name, tf_sec=60):
    ...
    for p in _pairs:
        if p[1] == pair_name: sid = p[0]; display = p[2]; payout = p[3]; max_payout = p[4]; break
            ...
            try:
                ...
                if isinstance(dj, list) and len(dj) > 0 and isinstance(dj[0], list) and len(dj[0]) >= 10:
                    ...
                    if fresh:
                        _pairs = fresh
                        for p in _pairs:
                            if p[1] == pair_name: payout = p[3]; max_payout = p[4]; break
                    pending = ""
                    continue
    ...
    if candles:
        ...
        candles.sort(key=lambda x: x["t"])
    return {"pair": pair_name, "display": display, "tf": tf_sec, "candles": candles, "ticks": len(ticks), "payout": payout, "max_payout": max_payout}
...
def get_data_tf(pair, tf):
    ...
    if result is None: return jsonify({"error": "Pair not found or fetch failed"}), 404
...
t.start()
if not init_done.wait(timeout=120):
    print("Init timed out (120s). Exiting.")
else:
    print(f"\nServer ready! http://localhost:5000")
    print(f"Try: http://localhost:5000/EURUSD_otc")
